...
def mega_login(email, password):
    print("[jframe] Logging in to mega")
    subprocess.run(['mega-login', email, password], check=True)
    print("[jframe] Login complete")

def mega_logout():
    print("[jframe] Logging out of mega")
    subprocess.run(['mega-logout'], check=True)
    print("[jframe] Logout complete")


def list_remote_files(remote_path):
    print(f"[jframe] Getting listing of files in mega:{remote_path}")
    result = subprocess.run(['mega-ls', '-l', remote_path], capture_output=True, text=True, check=True)
    print("[jframe] List received. Parsing list")
    files = {}
...
def download_file(remote_path, local_path):
    print(f"[jframe] Downloading mega:{remote_path} to local:{local_path}")
    subprocess.run(['mega-get', remote_path, local_path], check=True)
    print("[jframe] Download complete")
